...
- **GitHub Pages Deployment**
  - Build script for static export ✅ (`npm run build:gh-pages`)
  - GitHub Actions workflow for auto-deployment ✅ (`.github/workflows/gh-pages.yml`)
  - Environment-specific configuration ✅ (`NEXT_PUBLIC_BASE_PATH=/app-dieta`)
  - Pre-deployment testing scripts ✅ (`npm run export` local)
...
### 3.1 Diet Catalog System
- **Database Schema Implementation**
  - `diets` table with metadata
  - `diet_variants` for caloric variations
  - `meals` table for individual meal data
...
- **Recommendation Screens**
  - AI-suggested diets display
  - "Why recommended" explanations
  - Manual refresh functionality with cooldown timer

### 3.3 Diet Management
- **Diet Selection & Following**
  - "Follow Now" functionality
  - Current diet tracking
  - Diet switching capabilities
...
- **Meal Logging System**
  - Checkbox functionality for meal completion
...
### 5.1 Stripe Integration (Free Tier)
- **Payment Setup**
...
### 6.2 Leaderboards & Social Features
- **Ranking System (Screen 8.1)**
...
- **Social Elements**
  - User aliases and avatars
  - Privacy settings
  - Achievement sharing
  - Community engagement

### 6.3 Advanced Features
- **PDF Export (Screen 7.2)**
...
2. **User Creation Flow**:
   ...
   * On success, user is redirected to **Home (2.1)**.
...
* Successful sign-up → redirect to **Home (2.1)**.
...
* **Form Fields**:
  ...
  6. **Activity Level** (select, optional)
...
This is the **central hub of the application** where the user accesses the main modules of the diet SaaS. It provides quick navigation, highlights relevant information, and serves as the **entry point for all major features**.

**Main Goals**:

* Display personalized greetings and summary of user’s current status.
* Provide access to:

  * **Diet Catalog (4.1)**
  * **Current Diet / My Week (6.1)**
  * **Shopping List (7.1)**
  * **Ranking / Leaderboards (8.1)**
  * **User Hub / Eu (9.1)**
* Show **quick stats**: current weight, badges, adherence, etc.
* Optionally highlight **recommended diets** (AI-suggested) or alerts.
...
* **Header**
  ...
  * Logo / App name
  * Navigation menu (Diet Catalog, My Week, Shopping List, Rankings, Eu)
  * User avatar / shortcut to User Hub (9.1)
...
* **Main Dashboard Area**
  ...
  * **Quick Stats / Mini Cards**:
    ...
    * Current weight
    * Current diet adherence (progress this week)
    * Badges summary (number earned / total)
    * Quick CTA to “Update Weight”
  * **Suggested Diets Panel** (optional)

    * Show 3–5 AI-recommended diets based on Onboarding Questionnaire
    * Click → navigate to **Diet Detail (5.1)**
  * **Shortcut Buttons / Cards**:

    * “View All Diets” → **Diet Catalog (4.1)**
    * “My Week” → **My Week (6.1)**
    * “Shopping List” → **Shopping List (7.1)**
    * “Ranking” → **Ranking (8.1)**

* **Footer**

  * Legal links: Terms, Privacy, FAQ
  * Optional: Vercel analytics placeholder
...
1. **Dynamic Greeting**
   ...
   * Fetch user's first name from Supabase profile.
...
2. **Quick Stats**
   ...
   * Pull data from:
     ...
     * `profiles` table → weight
     * `user_diets` table → adherence / current diet
     * `badges` table → earned badges
   * Display in cards with minimal graphics (bar chart or numbers).

3. **Navigation**

   * Each card or button navigates to the appropriate screen.
   * Suggested diets card → diet detail (5.1) via AI recommendation.

4. **Responsiveness**

   * Works seamlessly on mobile and desktop.
   * Layout adjusts for cards, shortcuts, and quick stats.

5. **Onboarding Redirect**

   * If onboarding not completed (`profiles.onboarding_completed = false`) → redirect to **2.1 Onboarding**.

---

## Error Handling

* If fetching user data fails → show friendly error message, retry button.
* If AI recommendation fails → hide Suggested Diets panel and log error.
* Network issues: display offline notice.
...
* **Primary Navigation**:
  ...
  * Rankings → 8.1
  * Eu → 9.1
* Optional Quick Access: Suggested Diets → 5.1
...
* **Supabase Tables**:
  ...
  * `profiles` → first name, weight
  * `user_diets` → current diet, adherence
  * `badges` → earned / pending
  * `diets` → for suggested AI diets

* **Form State / Hooks**:

  * React `useEffect` for data fetching on load
  * `react-query` (optional) for caching and background refresh

* **UI Components**:

  * shadcn/ui Cards, Buttons, Avatar
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="quick-stats">
        ...
        <div>Current Diet: {profile.current_diet || "None"}</div>
        <div>Badges: {profile.badges_count}</div>
      </div>
      <div className="shortcut-buttons">
        <button onClick={() => window.location.href="/diets"}>View All Diets</button>
        <button onClick={() => window.location.href="/my-week"}>My Week</button>
        <button onClick={() => window.location.href="/shopping-list"}>Shopping List</button>
        <button onClick={() => window.location.href="/ranking"}>Ranking</button>
      </div>
      <div className="suggested-diets">
        <h2>Recommended for You</h2>
        {/* Map AI-suggested diets here */}
      </div>
    </div>
  );
}
...
* Quick stats accurately reflect profile, current diet, and badges.
* Navigation buttons take user to the correct modules.
* Suggested diets (AI) are shown if available.
...
* Only logged-in user can access their dashboard.
* Data fetched is strictly scoped to `auth.user.id` via Supabase RLS.
* No sensitive info exposed in the front-end console.
...
* Middleware:
  ...
  * Redirect users who haven’t completed onboarding → `/onboarding`
...
* `@supabase/supabase-js` → fetch user data
* `shadcn/ui` → Cards, Buttons, Avatar
...
```sql
select * from profiles where id = '<user_id>';
select * from user_diets where user_id = '<user_id>';
select count(*) from badges where user_id = '<user_id>';
```
...
* **Suggested / Highlighted Section (Optional)**

  * At the top, “Recommended for You” showing 3–5 diets flagged by the AI recommendation system.
...
* **Supabase Tables**
  ...
  * `diet_recommendations` (optional) → AI-suggested diets per user
* **UI Components**
...
* Allow users to select a **caloric variation** if pre-calculated options exist.
...
* **Header**
  ...
  * Diet name
  * Category badges
  * Back button → navigates to previous screen (Catalog, Favorites, or Week Plan)

* **Caloric Options / Pre-Calculated Variations**

  * Dropdown or buttons to select caloric version (e.g., 1500 kcal, 1800 kcal, 2000 kcal)
  * Selecting a variation updates meal macros and totals
...
* **Meal Substitutions** (Pop-up/Expandable)
  ...
  * Triggered by “Swap” or “Suggest Alternative” button on each meal item
...
1. **Diet Fetching**
   ...
   * Fetch related meals and pre-calculated caloric variations

2. **Follow Now Logic**

   * On button click:

     * Update `user_current_diet` table with selected `diet_id` and caloric variation
     * If user already has an active diet, replace it

3. **Meal Substitutions**

   * Fetch from `meal_alternatives` table based on meal\_id
   * Limit suggestions to maintain macro/calorie equivalence

4. **Caloric Variation Switch**
...
* **Supabase Tables**
  ...
  * `diets` → main diet metadata
  * `diet_meals` → meals per diet (meal\_id, day, order)
  * `meal_alternatives` → alternative ingredients or recipes
  * `user_current_diet` → tracks the user’s active diet and selected variation
...
export default function DietDetail({ dietId }) {
  ...
  useEffect(() => {
    async function fetchDiet() {
      const { data, error } = await supabase
        .from("diets")
        .select("*, diet_meals(*), pre_caloric_variations(*)")
        .eq("id", dietId)
        .single();
      if (!error) {
        setDiet(data);
        setSelectedCalories(data.pre_caloric_variations[0].calories);
      }
    }
    fetchDiet();
  }, [dietId]);
  ...
  async function followNow() {
    await supabase
      ...
      .upsert({ user_id: supabase.auth.user().id, diet_id: diet.id, calories: selectedCalories });
  ...
  return (
    <div className="diet-detail">
      ...
      {diet.diet_meals.map(meal => (
        <div key={meal.id} className="meal-card">
          <h2>{meal.name} ({meal.type})</h2>
          <p>{meal.ingredients}</p>
          <p>{meal.macros.protein}P / {meal.macros.carbs}C / {meal.macros.fat}F</p>
          <button>Swap</button>
        </div>
      ))}
...
* All meals, macros, and calorie variations display correctly.
* User can switch caloric variation and see updated values.
...
```sql
-- Fetch diet with meals and variations
select d.*, dm.*, v.*
from diets d
join diet_meals dm on dm.diet_id = d.id
join pre_caloric_variations v on v.diet_id = d.id
where d.id = '<diet_id>';

-- Set diet as current for user
upsert into user_current_diet(user_id, diet_id, calories)
values ('<user_id>', '<diet_id>', <selected_calories>);
```
...
# Screen ID: 6.1 – Minha Semana / Current Plan

## Overview

The **Minha Semana / Current Plan** screen shows the **user’s active diet plan** for the current week. It acts as a **central hub** for daily meal tracking, adherence logging, and quick access to the shopping list. Users can also **switch to another diet** from here (redirect to Diet Catalog – 4.1).

**Main Goals:**

* Display current week’s meal plan with clear structure (days and meals).
* Allow users to mark meals as **completed** (checkbox).
...
* **Week Overview / Calendar**
  ...
  * Each day displayed horizontally or vertically
  * Shows daily total calories and macro summary
  * Click a day → expand meals for that day
...
* **Progress Summary**
  ...
  * Optional small bar showing completion percentage of meals logged for the week
...
1. **Fetch Active Diet**
   ...
   * Fetch related meals from `diet_meals` table with caloric variation

2. **Meal Adherence Logging**

   * Each meal checkbox updates `user_meal_log` table:

     ```json
     { user_id, diet_id, meal_id, date, completed: true/false }
     ```
   * Persist across sessions

3. **Weekly Aggregation**

   * Calculate total calories consumed (based on completed meals)
   * Calculate % adherence per day and per week
...
5. **Navigation**
   ...
   * Back → Home (3.1)
   * Click day → expand meals
   * Meal → navigate to Diet Detail (5.1)

---

## Error Handling

* No active diet → show empty state with CTA: “Choose a diet now”
* Meal log update fails → toast: “Failed to update meal. Try again.”
...
* **Supabase Tables**
  ...
  * `user_current_diet` → active diet
  * `diet_meals` → meal details
  * `user_meal_log` → stores meal completion for adherence
* **UI Components**
...
* **Performance**
  ...
  * Lazy-load meal details on day expansion
  ...
  useEffect(() => {
    async function fetchPlan() {
      const { data } = await supabase
        .from("user_current_diet")
        .select("*, diet_meals(*), user_meal_log(*)")
        .eq("user_id", supabase.auth.user().id)
        .single();
      setMeals(data.diet_meals);
      // Initialize adherence state
      const log = {};
      data.user_meal_log.forEach(m => log[m.meal_id] = m.completed);
      setAdherence(log);
    }
    fetchPlan();
  }, []);
        ...
        <div key={day.day} className="day-card">
          ...
          {day.meals.map(meal => (
            <div key={meal.id} className="meal-card">
              ...
              <input
                type="checkbox"
                checked={adherence[meal.id] || false}
...
* Display current week with all meals correctly.
* User can toggle meal completion → updates `user_meal_log`.
...
* Row-level security enforced in `user_meal_log` table.
...
```sql
-- Fetch current diet with meals and user log
select u.*, dm.*
from user_current_diet u
join diet_meals dm on dm.diet_id = u.diet_id
left join user_meal_log l on l.meal_id = dm.id and l.user_id = u.user_id
where u.user_id = '<user_id>';

-- Update meal adherence
upsert into user_meal_log(user_id, diet_id, meal_id, date, completed)
values ('<user_id>', '<diet_id>', '<meal_id>', '<date>', <completed>);
```
...
The **Shopping List** screen generates a **weekly grocery list** based on the user’s currently selected diet. The list aggregates all ingredients from the diet’s meals, calculates total quantities, and allows the user to **check off items**, adjust portions, and export the list as PDF.

**Main Goals:**

* Display a clear, categorized list of groceries for the week.
* Allow **checkbox functionality** for tracking items purchased.
* Allow **portion adjustments** (optional MVP).
...
* **Ingredient Categories**
  ...
  * Group items by type (e.g., Vegetables, Proteins, Grains, Dairy)
  * Each category collapsible/expandable

* **Ingredient Items**

  * Name of ingredient (e.g., “Chicken breast”)
  * Quantity (sum of all meals for the week)
  * Unit (grams, liters, pieces)
  * Checkbox for tracking purchased items
  * Optional: adjust quantity for family or portion scaling (MVP: can be omitted or “coming soon”)

* **Actions**

  * Export / Print PDF → navigates to 7.2
  * Optional: Clear all checkboxes
...
1. **Fetch Current Diet Ingredients**
   ...
   * Query `user_current_diet` to retrieve selected diet
   * Fetch all meals → aggregate ingredients by type and sum quantities

2. **Checkbox / Completion Tracking**

   * Local state or persisted per user (`user_shopping_progress`)
   * Optional: reset weekly automatically

3. **Export / Print**

   * Export current list to PDF (7.2)
   * Keep layout simple for printing
...
* **Supabase Tables**
  ...
  * `user_current_diet` → selected diet and meals
  * `diet_meals` → recipes with ingredients and quantities
  * Optional: `user_shopping_progress` → track checked items

* **UI Components**

  * shadcn/ui: Card, Checkbox, Button, Collapse/Accordion
  * Tailwind CSS for responsive layout

* **Performance**

  * Aggregate ingredients server-side if possible to reduce client computation
  ...
  useEffect(() => {
    async function fetchIngredients() {
      const { data } = await supabase
        .from("diet_meals")
        .select("*")
        .eq("diet_id", supabase.auth.user().current_diet_id);
      
      // Aggregate quantities by ingredient
      const aggregated = aggregateIngredients(data);
...
* Displays all ingredients aggregated by category
* Shows total quantities per ingredient
* Checkbox works for tracking purchased items
...
* `shadcn/ui` → Card, Checkbox, Button, Accordion
* Optional: `jsPDF` for PDF export

### Supabase SQL Checks

```sql
-- Fetch aggregated ingredients for current user diet
SELECT ingredient_name, SUM(quantity) as total_quantity, unit
FROM diet_meals
WHERE diet_id = (SELECT current_diet_id FROM user_current_diet WHERE user_id = '<user_id>')
...
The **Export / Print PDF** screen is responsible for generating a downloadable or printable version of the user’s **weekly shopping list**. This includes all items aggregated from their selected diet(s), grouped by market categories, with quantities adjusted to servings/family size.

**Main Goals:**

* Provide a clear, printable shopping list for offline use
* Allow users to download or print directly
* Ensure data matches the weekly diet selection (7.1 – Lista de Compras)
...
1. **Header**
   ...
   * Optional subtitle: “Based on your diet plan for this week”

2. **Shopping List Table / Card**

   * Grouped by category (e.g., Vegetables, Proteins, Grains)
   * Columns: Item, Quantity, Unit (e.g., grams, pieces, liters)
   * Optional: checkbox for each item (to mark purchased, non-functional in PDF but useful on-screen preview)

3. **Controls / Actions**

   * **Download PDF** → triggers PDF generation & download
   * **Print PDF** → triggers browser print dialog
   * **Back Button** → returns to 7.1 Lista de Compras

4. **Preview Section (optional)**

   * Show a preview of how the PDF will look
   * Allows user to review before export
...
1. **PDF Generation**
   ...
   * Include:
     ...
     * Header with week and diet name(s)
     * Table of items with quantities
     * Optional footnotes (e.g., substitutions, notes)
   * Generate **well-formatted, paginated PDF** if list is long

2. **Print**

   * Use browser `window.print()` with custom print stylesheet
   * Ensure table fits within page width, repeated header for multi-page

3. **Dynamic Data**

   * Aggregate shopping list from **7.1 – Lista de Compras**
   * Adjust quantities if the user changed servings/family size
   * Sort items by market category

4. **Responsive**

   * On mobile: show simplified table with essential columns
   * On desktop: show full table preview
...
* **Supabase Queries**
  ...
  * Fetch selected diet and recipes for the week:

  ```sql
  SELECT * FROM user_weekly_diet
  WHERE user_id = '<USER_ID>' AND week_start = '<CURRENT_WEEK>';
  ```

  * Aggregate ingredients from recipes:

  ```sql
  SELECT ingredient, SUM(quantity) as total_quantity, unit, category
  FROM recipe_ingredients
  WHERE recipe_id IN (<USER_WEEK_RECIPE_IDS>)
  GROUP BY ingredient, unit, category
...
* **Routing**
  ...
  * 7.1 Lista de Compras → click **Export PDF** → navigate to 7.2 Export screen (can be modal or separate route `/shopping-list/export`)
...
export default function ExportPDF({ items, weekLabel }: { items: ShoppingItem[], weekLabel: string }) {
  ...
  return (
    <div className="p-4 flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-4">Export Your Shopping List</h1>
      <Button onClick={generatePDF}>Download PDF</Button>
      <Button className="mt-2" onClick={() => window.print()}>Print</Button>
      <Button className="mt-2" variant="secondary" onClick={() => history.back()}>Back</Button>
    </div>
  );
}
```
...
* PDF is properly formatted, with categories, quantities, and units
...
# Screen ID: 8.1 – Ranking

## Overview

The **Ranking** screen displays **leaderboards of users** based on engagement and progress metrics. The goal is to **motivate users** through social comparison while maintaining privacy.

**Main Goals:**

* Show rankings by **consistency** (e.g., weekly adherence)
* Show rankings by **weight lost** (absolute and relative)
* Enable users to **see their own position**
* Maintain **privacy**: users can set display alias/avatar in `Eu > Privacy & Display`
...
* **Filter / Tabs**
  ...
  * Metric filter: Consistency, Weight Lost (absolute/relative)
...
* **Leaderboard List**
  ...
  * Optional avatar
  * Metric value (e.g., % adherence, kg lost)
  * Highlight **current user** in a different style

* **Empty / Placeholder State**

  * CTA: “Start your first diet to appear in rankings!”
...
1. **Fetch Leaderboard Data**
   ...
   * Query `leaderboard_metrics` table (or dummy data initially)
   * Sort by selected metric and period

2. **Filter / Tab Change**

   * User selects a different period or metric → leaderboard refresh

3. **Highlight Current User**

   * Always show current user at bottom/top with a highlight even if not in top rankings

---

## Error Handling

* Fetch fails → toast: “Unable to load rankings. Try again later.”
* No users yet → show placeholder leaderboard with explanatory text
...
* From Ranking →
  ...
  * Back → Home (3.1)
  * Filter changes update same screen
...
* **Supabase Tables**
  ...
  * `user_metrics` → adherence %, weight lost
  * `leaderboard_metrics` → precomputed weekly/monthly leaderboard

* **UI Components**

  * shadcn/ui: Table, Tabs, Badge, Avatar
  * Tailwind CSS for responsive layout

* **Performance**

  * Precompute leaderboard weekly on server side to minimize client computation
  ...
  return (
    <div className="leaderboard">
      ...
      <Tabs value={metric} onChange={setMetric}>
        ...
        <Tab value="weight_lost">Weight Lost</Tab>
      </Tabs>
      {leaderboard.map(user => (
        <div key={user.user_id} className={`leaderboard-row ${user.is_current ? "highlight" : ""}`}>
          <span>{user.rank}</span>
          <span>{user.display_name}</span>
          <span>{user.metric_value}</span>
        </div>
      ))}
    </div>
  );
...
* Displays leaderboard with ranks, names/alias, avatar, and metric
* Supports period and metric filtering
...
* Users only see **display name / alias** for privacy
* No sensitive user data exposed
* Ensure RLS on `user_metrics` / `leaderboard_metrics` tables
...
* `@supabase/supabase-js` → fetch leaderboard data
* `shadcn/ui` → Tabs, Table, Avatar, Badge
* Tailwind CSS → layout

### Supabase SQL Checks

```sql
-- Fetch leaderboard for selected metric & period
SELECT rank, user_id, display_name, avatar_url, metric_value
FROM leaderboard_metrics
WHERE metric = 'consistency' AND period = 'week'
ORDER BY rank ASC;
```
...
# Screen ID: 9.1 – User Hub / Dashboard ("Eu")

## Overview

The **User Hub / Dashboard** is the central screen for the user to **overview their progress, profile, subscription, badges, and navigation** to key features. It serves as the main entry point to personal information and engagement tools.

**Main Goals:**

* Provide a **quick summary** of current weight, diet adherence, and badges.
* Display **subscription status** and access to plan management.
* Provide **shortcuts** to other core sections: My Diet, Shopping List, Rankings, Preferences, Profile.
* Central navigation hub for the user.
...
* **Header**
  ...
  * Title: “Dashboard” or “Eu”
  * Small welcome message with user’s first name

* **Weight Overview**

  * Current weight (latest entry from 9.2)
  * Mini line chart showing weekly or monthly progress
  * Button/shortcut to **Registro de Peso** (9.2)

* **Current Diet / Week Summary**

  * Name of active diet plan (6.1)
  * Button → **View Current Week / My Week** (6.1)

* **Badges Summary**

  * Display 2–3 recent badges / most important achievements
  * Click → navigate to **Minhas Badges** (9.3)

* **Subscription Status**

  * Show active plan (monthly/trimestral/semestral)
  * Next billing date
  * Shortcut → **Assinatura & Pagamentos** (9.6)

* **Quick Navigation Tiles / Shortcuts**

  * Dietas (4.1 / 5.1)
  * Shopping List (7.1)
  * Rankings (8.1)
  * Preferences / Onboarding revisits (2.1)

* **Footer / Support**

  * Link to FAQ / Contact (S2)
  * Link to Privacy & Terms (S3)
...
1. **Fetch Data**
   ...
   * Current user profile, weight log, active diet, badges, subscription status
   * Fetch summary data for charts (e.g., last 4 weeks)

2. **Dynamic Updates**

   * Update mini-chart when a new weight entry is added
   * Badges appear dynamically on event completion

3. **Navigation**

   * Each tile or button navigates to its corresponding screen
   * Consistent back navigation to Dashboard from sub-screens

4. **Responsive Layout**

   * Dashboard adapts to desktop and mobile
   * Collapsible or stacked tiles for smaller screens

---

## Error Handling

* Missing data → show placeholders (e.g., “No weight data yet. Add your first entry!”)
* Fetch fails → toast notification: “Unable to load dashboard. Retry later.”
...
* From Dashboard →
  ...
  * My Week / Current Diet (6.1)
  * Registro de Peso (9.2)
  * Minhas Badges (9.3)
  * Subscription & Payments (9.6)
  * Diet Catalog (4.1)
  * Shopping List (7.1)
  * Rankings (8.1)
...
* **Supabase Tables**
  ...
  * `users` → profile info, display name
  * `user_current_diet` → current diet plan
  * `user_metrics` → weight log, adherence
  * `user_badges` → badges earned
  * `user_subscription` → subscription details

* **UI Components**

  * shadcn/ui: Card, Button, Avatar, Tabs, Mini Chart
  * Tailwind CSS → responsive grid layout

* **Performance**

  * Aggregate data server-side where possible (e.g., weekly weight trends)
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: weights } = await supabase.from("user_metrics").select("*").eq("user_id", userId);
      const { data: badgesData } = await supabase.from("user_badges").select("*").eq("user_id", userId);
      const { data: subs } = await supabase.from("user_subscription").select("*").eq("user_id", userId).single();
  ...
  return (
    <div className="dashboard grid gap-4">
      ...
      <QuickNav tiles={["Dietas","Shopping List","Rankings","Preferences"]} />
    </div>
  );
}
```
...
* Displays current weight, mini-progress chart, badges, subscription info
* Shows actionable navigation tiles to main app sections
* Updates dynamically when new data is available
* Handles empty states gracefully
...
* Users only access their own data (RLS policies on Supabase)
* No sensitive data is exposed outside the authenticated session

---

## Next.js App Routing & Middleware

* Route: `/dashboard` (or `/eu`)
* Middleware:
...
* `@supabase/supabase-js` → fetch user profile, weights, badges, subscription
* `shadcn/ui` → Card, Avatar, Button, Tabs, Mini Chart
* Tailwind CSS → responsive layout

### Supabase SQL Checks

```sql
-- Fetch latest weight entry for user
SELECT weight, date
FROM user_metrics
WHERE user_id = '<user_id>'
ORDER BY date DESC
...
#Rule to implement:
* On the "Eu > Registro de Peso" screen, there is a button: **"Update diet recommendations using AI"**.
* Behavior:
  1. Button is disabled for **1 week** after each use.
  2. When clicked:
...
# Screen ID: 9.3 – User Badges ("Minhas Badges")

## Overview

The **User Badges screen** allows users to **view all badges they have earned** and see the **progress toward upcoming badges**. It gamifies adherence to diet plans and motivates continued engagement.

**Main Goals:**

* Display badges already earned and badges yet to be earned
* Provide **detailed descriptions** of each badge
* Show progress toward each badge (streaks, weight lost, diet adherence)
* Encourage engagement by making achievements visible
...
* **Header**
  ...
  * Title: “My Badges” or “Minhas Badges”
  * Back button → Dashboard (9.1)

* **Earned Badges Section**

  * Grid or list layout of badges earned
  * Badge icon (color or filled)
  * Badge title
  * Tooltip / expandable description
  * Optional: date earned

* **Upcoming / Locked Badges Section**

  * Greyed-out or locked icons
  * Display criteria to earn
  * Progress bar if applicable (e.g., 3/7 days streak)

* **Badge Detail Modal**

  * Opens on click of any badge
  * Shows full description, earned date, and progress
  * Close button → returns to Minhas Badges screen

* **Empty / Placeholder State**

  * “No badges earned yet. Start your first week to earn badges!”
...
1. **Fetch Data**
   ...
   * Query `user_badges` table for earned badges
   * Query `badges_catalog` table for all possible badges and criteria
   * Calculate progress for unearned badges dynamically

2. **Dynamic Updates**

   * Badges unlocked should appear in real-time if criteria met
   * Optional animation / toast on new badge

3. **Navigation**

   * Back → Dashboard (9.1)
   * Badge click → Badge Detail Modal (9.5)

4. **Responsive Layout**

   * Grid adapts to desktop and mobile
   * Scrollable for large number of badges

---

## Error Handling

* Badge fetch fails → toast: “Unable to load badges. Retry later.”
* Empty badge list → show placeholder text
...
* **Supabase Tables**
  ...
  * `badges_catalog` → `id`, `title`, `description`, `criteria`

* **UI Components**

  * shadcn/ui: Card, Badge, Grid, Modal, Progress Bar
  * Tailwind CSS → responsive layout

* **Performance**

  * Fetch all badges at once, compute progress client-side or via Supabase RPC if needed
...
export default function MyBadges() {
  ...
  useEffect(() => {
    async function fetchBadges() {
      const { data: earned } = await supabase
        ...
        .select("*")
        .eq("user_id", userId);
      const { data: all } = await supabase.from("badges_catalog").select("*");
  ...
  return (
    <div className="my-badges grid gap-4">
      <h1>My Badges</h1>
      <div className="earned-badges grid grid-cols-3 gap-2">
        {earnedBadges.map(b => (
          <BadgeCard key={b.badge_id} badge={b} />
        ))}
      </div>
      <div className="locked-badges grid grid-cols-3 gap-2">
        {allBadges
...
* Displays all earned badges with date earned
* Displays locked/unearned badges with progress and criteria
* Opens badge detail modal on click
* Handles empty states gracefully
* Updates dynamically when new badges are earned

---

## Security Considerations

* Users only access **their own badge data** (RLS policies on Supabase)
* Badge unlocking logic verified server-side if needed
...
* `@supabase/supabase-js` → fetch user badges and catalog
* shadcn/ui → Card, Badge, Grid, Modal, Progress Bar
* Tailwind CSS → responsive layout
...
SELECT * FROM badges_catalog;

-- Insert newly earned badge
...
3. **Dynamic Updates**
   ...
   * After password change, optionally force re-login or refresh session
...
The **Privacy & Display screen** allows users to manage how they appear to other users on public areas of the application, such as the **Ranking / Leaderboards**. Users can set a public alias (nickname) and optionally select an avatar. This ensures privacy while still enabling engagement in social features.
...
* **Public Alias Section**
  ...
  * Input field: "Public Alias"
  * Validation: unique across all users (optional for MVP, but preferred)
...
* **Buttons**
  ...
  * Save → persists changes to Supabase `users` table
  * Cancel → discards changes, returns to “Eu” hub
...
1. **Fetch Data**
   ...
   * Fetch current user alias and avatar from Supabase `users` table on component mount
...
* **Supabase Table**
  ...
  * `users` → `public_alias`, `avatar_id` (integer referencing predefined avatars)
...
* **Avatar Options**
  ...
  * MVP: 8–12 predefined avatar images
  * No user uploads
...
export default function PrivacyDisplay() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data, error } = await supabase.from("users").select("public_alias, avatar_id").eq("id", userId).single();
      if (error) Toast.error("Failed to load privacy settings.");
      else {
        setAlias(data.public_alias || "");
        setSelectedAvatar(predefinedAvatars[data.avatar_id] || predefinedAvatars[0]);
      }
      setLoading(false);
    }
    fetchData();
  ...
  const handleSave = async () => {
    const avatarIndex = predefinedAvatars.indexOf(selectedAvatar);
    const { error } = await supabase.from("users").update({ public_alias: alias, avatar_id: avatarIndex }).eq("id", userId);
  ...
  return (
    <div className="p-4">
      ...
      <Input label="Public Alias" value={alias} onChange={e => setAlias(e.target.value)} />
...
* Allows editing alias and selecting avatar
...
* `@supabase/supabase-js` → fetch/update `public_alias` and `avatar_id`
* shadcn/ui → Input, Button, Toast, Grid
* Tailwind CSS → styling
...
SELECT public_alias, avatar_id FROM users WHERE id = '<user_id>';

-- Update privacy settings
UPDATE users
SET public_alias = '<alias>', avatar_id = <avatar_index>
WHERE id = '<user_id>';
```
